...
router.get("/", async (req, res) => {
    try {
        ...
        const countResult = await (0, database_1.query)(`SELECT COUNT(*) FROM provider_profiles pp
       JOIN users u ON pp.user_id = u.id
       WHERE pp.is_verified = true`);
        const result = await (0, database_1.query)(`SELECT u.id, u.first_name, u.last_name, u.email, pp.business_name, pp.description,
...
router.get("/:id", async (req, res) => {
    try {
       ...
       WHERE u.id = $1`, [id]);
        ...
        // Get services
        const servicesResult = await (0, database_1.query)(`SELECT s.*, c.name as category_name
       FROM services s
       JOIN categories c ON s.category_id = c.id
       WHERE s.provider_id = $1 AND s.is_active = true`, [id]);
        res.json({
            success: true,
            data: {
                ...result.rows[0],
...
router.put("/profile", auth_1.authenticate, async (req, res) => {
    try {
        ...
        const { businessName, description, experienceYears, location, profileImage, } = req.body;
        const result = await (0, database_1.query)(`UPDATE provider_profiles
...
router.get("/:id/reviews", async (req, res) => {
    try {
       ...
       LIMIT $2 OFFSET $3`, [id, Number(limit), offset]);
        const countResult = await (0, database_1.query)("SELECT COUNT(*) FROM reviews WHERE provider_id = $1", [id]);
        res.json({
            success: true,
            data: result.rows,
            pagination: {
