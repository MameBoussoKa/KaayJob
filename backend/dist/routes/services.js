...
router.get("/", async (req, res) => {
    try {
        ...
        const countResult = await (0, database_1.query)(`SELECT COUNT(*) FROM services s ${whereClause}`);
        const result = await (0, database_1.query)(`SELECT s.*, c.name as category_name, u.first_name, u.last_name, pp.business_name, pp.rating
       FROM services s
...
router.get("/:id", async (req, res) => {
    try {
        const { id } = req.params;
        const result = await (0, database_1.query)(`SELECT s.*, c.name as category_name, c.id as category_id,
...
router.post("/", auth_1.authenticate, [
    ...
    (0, express_validator_1.body)("description")
        ...
        .isLength({ max: 1000 }),
    (0, express_validator_1.body)("categoryId").isInt({ min: 1 }).withMessage("Catégorie invalide"),
    (0, express_validator_1.body)("price").isFloat({ min: 0 }).withMessage("Prix invalide"),
    ...
    try {
        ...
        const result = await (0, database_1.query)(`INSERT INTO services (provider_id, category_id, name, description, price, duration, is_active)
       VALUES ($1, $2, $3, $4, $5, $6, true)
       RETURNING *`, [user.id, categoryId, name, description, price, duration]);
        res
            .status(201)
...
router.put("/:id", auth_1.authenticate, async (req, res) => {
    try {
        ...
        const user = req.user;
        // Verify ownership
        const existing = await (0, database_1.query)("SELECT provider_id FROM services WHERE id = $1", [id]);
        ...
        const { name, description, categoryId, price, duration, isActive } = req.body;
        const result = await (0, database_1.query)(`UPDATE services
       SET name = COALESCE($1, name),
           ...
           updated_at = NOW()
       WHERE id = $7
       RETURNING *`, [name, description, categoryId, price, duration, isActive, id]);
        res.json({
            success: true,
